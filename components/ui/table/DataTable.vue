<template>
  <div class="table-component">
    ...
    <div
      class="d-flex flex-wrap align-items-center justify-content-between mg-b-30 gap-15"
    >
      ...
      <btn
        type="btn-default-outline"
        size="lg"
        icon="ri-download-2-line tx-18"
        v-if="this.export"
        @clicked="exportToCsv"
        >Export to CSV
      </btn>
    ...
    <div v-if="filteredData.length">
        ...
        <thead>
          <tr>
            <th v-for="header in headers" :key="header.text">
              <div
                class="d-flex table-header gap-10"
              ...
              >
                ...
                <span
                  ...
                  class="d-flex align-items-center sort-arrow"
                >
                  <template v-if="sortOrders[header.value] === 1">
      ...
      <div class="table-footer pd-15 bd-t d-flex justify-content-between">
        <p v-if="showTotalRecords" class="tx-12 op-8">
          Total Records:
          <span class="tx-danger tx-medium">{{ filteredData.length }}</span>
        </p>
    ...
    <no-results
      v-else-if="!filteredData.length && searchKey.length > 0"
      :tx-size="14"
      :size="160"
      msg="No matching results found for your query"
    ></no-results>

    <no-results
      v-else-if="!filteredData.length && !searchKey.length"
      :tx-size="14"
      :size="160"
      msg="No Records Add yet, "
    ></no-results>
  </div>
</template>
  ...
  methods: {
    ...
    exportToCsv() {
      ...
      const dataRows = this.data
        .map((row) => this.headers.map((header) => row[header.value]).join(','))
        .join('\n')
