  ...
  <div class="carousel" ref="carousel">
    <div class="carousel-wrap" :style="`width: ${carouselClientWidth}px`">
      <div
        ...
        :style="{
          gap: gap + 'px',
          transform: 'translateX(' + -eachSlideWidth * currentSlide + 'px',
          width: eachSlideWidth * slidesCount + 'px'
        }"
      >
        <flex
          class="slide"
          v-for="(slide, index) in slides"
          :style="`width: ${eachSlideWidth}px`"
        >
    ...
    <flex
      ...
      content="center"
      items="center"
      class="carousel-nav mg-t-10"
      gap="15"
    >
      <btn icon="ri-arrow-left-s-line" @click.prevent="prevSlide"></btn>
      <div class="slider-counter">
        <p>
          <span class="count-current">{{ currentSlide + 1 }}</span
          ><span class="count-separator">/</span
          ><span class="count-total">{{ slidesCount }}</span>
        </p>
      </div>
...
export default {
  ...
  mounted() {
    ...
    this.slideWidth
      ? (this.eachSlideWidth = this.slideWidth)
      : (this.eachSlideWidth = this.carouselClientWidth)
  },
  methods: {
    nextSlide() {
