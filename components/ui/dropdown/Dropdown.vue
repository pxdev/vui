<template>
  <div class="dropdown search-dropdown" v-if="loading">
    <div class="dropdown-btn gap-5 d-flex align-items-center">
      <i class="ri-loader-5-line spin tx-20"></i> {{ $t('Loading') }}...
    </div>
  </div>

  <div
    v-else
    class="dropdown search-dropdown tx-pointer"
    ref="dropdownElement"
    :class="{ shown: dropdownOpen }"
  >
    <a
      href="#"
        ...
        <vue-custom-scrollbar class="dropdown-scroll">
          ...
          <div class="tx-center pd-5" v-else>
            <no-results
              :tx-size="14"
              :size="80"
              :msg="$t('No matching results found for your query')"
            ></no-results>
          </div>
...
export default {
  ...
  setup: function (props, { emit }) {
    ...
    watchEffect(() => {
      if (Escape.value) dropdownOpen.value = false
    })
